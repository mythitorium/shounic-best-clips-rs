...
#[derive(Serialize, Debug)]
struct OutgoingGetResponse { videos: Vec<Video>, c: i64 }
...
pub fn handle_get(request: &Request, db: &mut Transaction, uid: i64, state: &mut State) -> Response {
    ...
    match prep_votable_videos(&db, c, uid, state.videos_per_vote()) {
        DbResult::Ok(videos) => {
            return Response::json(&OutgoingGetResponse { videos: videos, c });
        },
        DbResult::Err(error) => {
            println!("{:?}", error);
        ...
        DbResult::Empty => {
            return Response::text("No more things").with_status_code(204);
        }
    }
}

// Picks videos at random that arn't in the banned or disqualified tables. Is tolerant to an inadequate number of videos
fn prep_votable_videos(db: &Transaction, category: i64, uid: i64, amount: i64) -> DbResult<Vec<Video>, Error> {
    let videos;
...
pub fn handle_post(request: &Request, db: &mut Transaction, uid: i64, state: &mut State) -> Response {
    ...
    match || -> Result<Vec<i64>, Error> {
        let mut stmt = db.prepare(QUERY_GET_ACTIVE_VOTE_VIDEOS)?;
        let mut rows = stmt.query([uid])?;
        let mut active_list: Vec<i64> = Vec::new();
        while let Some(row) = rows.next()? {
            active_list.push(row.get(0)?);
        }
        Ok(active_list)
    }() {
        Ok(active_list) => {
